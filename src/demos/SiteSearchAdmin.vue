  ...
  <div>
    ...
    />
      <div class="search-list-container">
        <div v-for="searchData in limitedResult" :key="searchData.name" class="search-list-result">
{{ searchData.name }}
        </div>
      </div>
      <div @click="toggleListSearch" class="switch-link"><i class="s-icon-link icon"></i>Or search by list</div>
      <div v-show="listSearch"  class="search-list-container">
        <div v-for="datum in jsonData" :key="datum.name" class="search-list-result" @click="selectCurrent(datum)">{{ datum.name }}</div>
      </div>
    </div>
    <div class="section">
      <div class="edit-grid-container" v-for="modify in jsonData" :key="modify.name" v-show="modify.name === currentSelection">
        <div class="left">Name:</div>
        <div class="right"><input type="text" :placeholder="modify.name"/></div>
        <!-- -->
        <div class="left">Title:</div>
        <div class="right"><input type="text" :placeholder="modify.title"/></div>
        <!-- -->
        <div class="left">Route:</div>
        <div class="right"><input type="text" :placeholder="modify.route"/></div>
        <!-- -->
        <div class="left">Keywords:</div>
        <div class="right"><input type="text" :placeholder="modify.keywords"/></div>
        <!-- -->
        <div class="left">Description:</div>
        <div class="right"><input type="text" :placeholder="modify.description"/></div>
        <!-- -->
        <div class="left">Image</div>
        <div class="right"><input type="text" :placeholder="modify.image"/></div>
        <!-- -->
        <div class="left">Howto</div>
        <div class="right"><input type="text" :placeholder="modify.howto"/></div>
        <!-- -->
        <div class="left">keymatch:</div>
        <div class="right"><input type="text" :placeholder="modify.keymatch"/></div>
        <!-- -->
        <div class="left">weight:</div>
        <div class="right"><input type="text" :placeholder="modify.weight"/></div>
        <!-- -->
...
modifyJson() {
  ...
  let insertObject: any = this.jsonData.find(data => data.name === this.currentSelection);
  ...
  fs.writeFileSync('./../components/sitesearchdata.json', stringedJson, { encoding: 'utf8'});
